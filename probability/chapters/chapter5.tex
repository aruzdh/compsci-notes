...
\qs{Distribuciones}{
  Demostrar que todas las funciones de probabilidad dadadas en esta sección son, efectivamente, funciones de probabilidad.
}
...
\qs{Distribución Exponencial}{
  Dada $\mcK \sim exp(\lambda)$. Calcular
  \[F_{\mathcal X}(x) = \int_{-\infty}^x f(r)~dr\]
}

\qs{Distribución Exponencial}{
  Usando el ejercicio anterior, verificar que la variable aleatoria exponencial satisface lo siguiente.
  \[P(\mathcal X > a + b) = P(\mathcal X > a)~P(\mathcal X > b)\]
}
...
\qs{Distribuciones}{
  Si $\mathcal X\sim N(\mu,\sigma^{2})$ Encuentra la densidad de $Y=e^{\mathcal X}$
}
...
\qs{Distribuciones}{
  Demostrar las propiedades de la Función Gamma.
}
