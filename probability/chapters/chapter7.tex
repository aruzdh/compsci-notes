...
\dfn{Varianza}{
  ...
  \[\text{Var}(\mathcal X) = \mathbb E([\mathcal X - \mathbb E(\mathcal X)]^2)\]
  Cuando dicha esperanza existe.
  \newpara
  De este modo, existen dos casos.
  \begin{itemize}
    \item Caso discreto.
      \[\text{Var}(\mathcal X) = \mathbb E([\mathcal X - \mathbb E(\mathcal X)]^2) = \sum_i (i - \mathbb E(\mathcal X))^2 P(\mathcal X = i)\]
    \item Caso continuo.
...
\ex{Varianza}{
  Calcular $\text{Var}(\mathcal X)$ si $\mathcal X \sim \text{exp}(\lambda)$
...
\ex{Esperanza y Varianza}{
  Sea $X \sim \Gamma(\alpha, \lambda)$. Calcula $\mathbb{E}(X)$ y $\text{Var}(X)$.
  \begin{itemize}
  \item Por un lado
  \begin{align*}
  ...
  \item Por otro lado para calcular $\text{Var}(X)$ primero calculamos lo siguiente,
  \begin{align*}
