...
#definition(title:"Linear Combination")[
  Let $V$ be a vector space and $S != emptyset subset.eq V$. A vector $v in V$ is called a *linear combination* of vectors of $S$ if there exist a finite number of vectors $u_1, u_2, dots, u_n in S$ and scalars $lambda_1, lambda_2, dots, lambda_n in KK$ such that $ v = lambda_1 u_1 + lambda_2 u_2 + dots lambda_n u_n $
