...
Suppose that $V$ is a vector space over an infinite field and that $W <= V$. Unless $W$ is the zero subspace, $W$ is an infinite set. It is desirable to find a "small" finite subset $S$ that generates $W$ because we can describe each vector in $W$ as a linear combination of the finite number of vectors in $S$. The search for this subset is related to the question of whether or not some vector in $S$ is a linear combination of the other vectors in $S$.
